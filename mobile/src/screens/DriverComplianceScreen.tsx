...
export function DriverComplianceScreen() {
  ...
  const dvirDraftKey = vehicleId && template ? secureDraftKey("driver-dvir", session?.company.id ?? session?.company.code, session?.user.id, `${vehicleScope}-${String(template.id ?? template.templateName ?? "template")}`) : null;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const action = hasDraft ? writeSecureDraft<DvirDraft>(dvirDraftKey, { results, attested, idempotencyKey }) : clearSecureDraft(dvirDraftKey);
  ...
  const toggle = (item: JsonRecord, index: number, result: "pass" | "fail") => {
    ...
    setResults((currentResults) => ({
      ...currentResults,
      [key]: { result, severity: result === "fail" ? currentResults[key]?.severity ?? "major" : "minor", notes: currentResults[key]?.notes ?? "" },
    }));
  };
  ...
  return (
    <Screen>
      <DriverSceneHero
        ...
        title={blocked ? "Stop. Resolve before moving." : lowDrive ? "Time needs attention." : "Ready for the road."}
        description="Vehicle blocking, DVIR, coaching, and HOS data stay identity-scoped. Missing certified HOS data is shown as unavailable—not guessed."
        status={blocked ? "Vehicle blocked" : lowDrive ? "Time attention" : "Driver ready"}
        tone={blocked ? "red" : lowDrive ? "amber" : "teal"}
      >
        <Row>
          <DriverActionTile code="V" title="Vehicle" subtitle={textOf(vehicleCode, "Unassigned")} tone={blocked ? "red" : "teal"} />
          <DriverActionTile code="H" title="HOS" subtitle={hos.data?.dataAvailable === false ? "No certified feed" : `${textOf(hos.data?.remainingDriveHours)}h drive left`} tone={lowDrive ? "amber" : "blue"} />
          <DriverActionTile code="D" title="DVIR" subtitle={completed ? "Checklist complete" : "Inspection required"} tone={completed ? "green" : "violet"} />
          <DriverActionTile code="C" title="Coaching" subtitle={`${String(coaching.data?.pendingCount ?? 0)} pending`} tone={(coaching.data?.pendingCount ?? 0) > 0 ? "amber" : "green"} />
        </Row>
      </DriverSceneHero>
      ...
      <Panel variant="elevated" tone={lowDrive ? "amber" : "blue"}>
        <SectionHeader eyebrow="Hours of service" title={textOf(hos.data?.hosStatus, "ELD status unavailable")} description="HOS is shown only when the backend has a paired driver record and usable source data." />
      ...
      <Panel variant="solid" tone="teal">
        <SectionHeader eyebrow="Pre-trip DVIR" title={textOf(template?.templateName, "Inspection checklist")} description="Every item must be marked. Critical failures can immediately place the vehicle out of service. Draft state and retry identity are encrypted until submission succeeds." right={<Pill label={textOf(vehicleCode, "No vehicle")} tone={vehicleId ? "teal" : "red"} />} />
        ...
        <View style={{ gap: 10 }}>
          {items.map((item, index) => {
            ...
            return (
              <View key={key} style={{ gap: 9, padding: 13, borderRadius: 18, borderWidth: 1, borderColor: value?.result === "fail" ? `${colors.red}66` : colors.border, backgroundColor: value?.result === "fail" ? `${colors.red}0d` : "rgba(255,255,255,0.028)" }}>
                ...
                {value?.result === "fail" ? <>
                  <Row>{(["minor", "major", "critical"] as const).map((severity) => <ActionButton key={severity} label={titleCase(severity)} onPress={() => setResults((all) => ({ ...all, [key]: { ...all[key], severity } }))} variant={value.severity === severity ? "secondary" : "ghost"} />)}</Row>
        ...
        {items.length ? <>
          ...
          <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: attested }} onPress={() => setAttested((value) => !value)} style={{ flexDirection: "row", gap: 10, alignItems: "flex-start", paddingVertical: 4 }}>
            <View style={{ width: 26, height: 26, borderRadius: 8, borderWidth: 1, borderColor: attested ? colors.teal : colors.borderStrong, backgroundColor: attested ? colors.teal : "rgba(255,255,255,0.02)", alignItems: "center", justifyContent: "center" }}><Text style={{ color: colors.backgroundDeep, fontWeight: "900" }}>{attested ? "✓" : ""}</Text></View>
