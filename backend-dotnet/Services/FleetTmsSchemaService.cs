...
public sealed class FleetTmsSchemaService(Database db, ILogger<FleetTmsSchemaService> log)
{
    public async Task EnsureAsync()
    {
        ...
        await MigrateSaudiFleetOperations();
        await EnsureWorkspaceIntegrity();
        await CreateIndexes();
    }

    // Canonical fleet attributes used by Saudi operators to schedule owned, rented,
    // and partner vehicles from one roster. Provider/government states are recorded
    // evidence only; a value of Pending or Not configured never implies a live TAMM,
    // Locus, or payment-provider connection.
    private async Task MigrateSaudiFleetOperations()
    {
        const string sql = @"
...
";
        ...
        {
            log.LogError(ex, "[FleetTmsSchema] Saudi fleet operations migration failed");
            throw;
        }
    }
