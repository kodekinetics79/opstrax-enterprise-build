...
public sealed class Batch7SchemaService(Database db)
{
    ...
    [
        ...
        @"INSERT INTO ai_recommendations (company_id, tenant_id, recommendation_type, module_key, title, summary, body, confidence_score, urgency_score, impact_json, reason_json, proposed_action_json, risk_level, status, score, description, priority, action_label, action_type) VALUES
          (1,1,'report_action','reports-analytics','Run SLA At-Risk Report — 6 Customers Flagged','6 customers have SLA risk scores above 50. Run the SLA At-Risk Report to identify breach root causes.','6 customers have SLA risk scores above 50. Run the SLA At-Risk Report to identify breach root causes.',91,88,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'High','draft',91,'6 customers have SLA risk scores above 50. Run the SLA At-Risk Report to identify breach root causes.','High','Run Report','report_action'),
          (1,1,'report_action','reports-analytics','Cost Leakage Report — $14,280 Recoverable','Cost leakage analysis shows $14,280 in recoverable savings this period. Export the Cost Leakage Report for Finance review.','Cost leakage analysis shows $14,280 in recoverable savings this period. Export the Cost Leakage Report for Finance review.',88,84,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'High','draft',88,'Cost leakage analysis shows $14,280 in recoverable savings this period. Export the Cost Leakage Report for Finance review.','High','Run Report','report_action'),
          (1,1,'schedule_action','reports-analytics','Schedule Daily HOS Warning Report','Drivers 2 and 4 have recurring HOS issues. Enable the Daily HOS Warning scheduled report to catch violations earlier.','Drivers 2 and 4 have recurring HOS issues. Enable the Daily HOS Warning scheduled report to catch violations earlier.',76,72,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'Medium','draft',76,'Drivers 2 and 4 have recurring HOS issues. Enable the Daily HOS Warning scheduled report to catch violations earlier.','Medium','Schedule Report','schedule_action'),
          (1,1,'report_action','reports-analytics','Executive Summary Report — Board Ready','Monthly executive summary is due. Run and export the Executive Operations Summary for board distribution.','Monthly executive summary is due. Run and export the Executive Operations Summary for board distribution.',72,68,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'Medium','draft',72,'Monthly executive summary is due. Run and export the Executive Operations Summary for board distribution.','Medium','Run Report','report_action'),
          (1,1,'kpi_action','sla-kpi','On-Time Delivery Drifting — 7.6% Below Target','OTD has dropped from 94% to 87.4% over 5 days. Investigate dispatch scheduling and vehicle availability.','OTD has dropped from 94% to 87.4% over 5 days. Investigate dispatch scheduling and vehicle availability.',96,94,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'Critical','draft',96,'OTD has dropped from 94% to 87.4% over 5 days. Investigate dispatch scheduling and vehicle availability.','Critical','Review KPI','kpi_action'),
          (1,1,'kpi_action','sla-kpi','Idle Cost Breaching Weekly Target — Review Top 3 Vehicles','Idle cost is $347.50/week over target. TRK-101, VAN-106, and BOX-107 account for 68% of idle time.','Idle cost is $347.50/week over target. TRK-101, VAN-106, and BOX-107 account for 68% of idle time.',89,86,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'High','draft',89,'Idle cost is $347.50/week over target. TRK-101, VAN-106, and BOX-107 account for 68% of idle time.','High','Review Idle Cost','kpi_action'),
          (1,1,'sla_action','sla-kpi','4 SLA Breaches Open — Customer Communications Required','SLA records SLA-003, SLA-007, SLA-010, and SLA-021 are in breach status. Customer notifications are overdue.','SLA records SLA-003, SLA-007, SLA-010, and SLA-021 are in breach status. Customer notifications are overdue.',87,84,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'High','draft',87,'SLA records SLA-003, SLA-007, SLA-010, and SLA-021 are in breach status. Customer notifications are overdue.','High','View SLA Breaches','sla_action'),
          (1,1,'kpi_action','sla-kpi','Gross Margin Below Target — 4 Routes Operating Below 10%','Gross margin at 17.4% vs 22% target. Identify low-margin routes and renegotiate customer rates.','Gross margin at 17.4% vs 22% target. Identify low-margin routes and renegotiate customer rates.',84,80,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'High','draft',84,'Gross margin at 17.4% vs 22% target. Identify low-margin routes and renegotiate customer rates.','High','Margin Analysis','kpi_action'),
          (1,1,'audit_action','audit-logs','97 Audit Events This Period — Coverage Score 91%','Audit coverage is near target. 23 recent actions pending enrichment. Review critical and warning severity events.','Audit coverage is near target. 23 recent actions pending enrichment. Review critical and warning severity events.',74,70,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'Medium','draft',74,'Audit coverage is near target. 23 recent actions pending enrichment. Review critical and warning severity events.','Medium','View Audit Logs','audit_action'),
          (1,1,'audit_action','audit-logs','2 Critical Audit Events Require Review','Audit events: ELD malfunction (eld.malfunction) and DVIR critical defect (dvir.critical_defect) are pending review.','Audit events: ELD malfunction (eld.malfunction) and DVIR critical defect (dvir.critical_defect) are pending review.',85,82,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'High','draft',85,'Audit events: ELD malfunction (eld.malfunction) and DVIR critical defect (dvir.critical_defect) are pending review.','High','Review Critical Events','audit_action'),
          (1,1,'audit_action','audit-logs','Compliance Audit Export Ready','AUD-2026-US-001 audit package is ready. Request an audit log export covering the same date range for the compliance review.','AUD-2026-US-001 audit package is ready. Request an audit log export covering the same date range for the compliance review.',71,68,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'Medium','draft',71,'AUD-2026-US-001 audit package is ready. Request an audit log export covering the same date range for the compliance review.','Medium','Export Audit Logs','audit_action'),
          (1,1,'executive_action','executive','Operations in WATCH Status — 3 KPIs Require Action','OTD, Idle Cost, and Gross Margin are below target. Recommend executive review and resource reallocation.','OTD, Idle Cost, and Gross Margin are below target. Recommend executive review and resource reallocation.',97,95,'{}'::jsonb,'{}'::jsonb,'{}'::jsonb,'Critical','draft',97,'OTD, Idle Cost, and Gross Margin are below target. Recommend executive review and resource reallocation.','Critical','View Executive Summary','executive_action'),
