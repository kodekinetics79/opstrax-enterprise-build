...
public sealed class CameraProviderStatusService(Database database)
{
        ...
        CancellationToken ct = default)
    {
        ...
        return await database.RunInSystemTransactionAsync(async () =>
        {
            ...
            // Provider event counts alone cannot describe the connector. In
            // particular, a denied camera scope can produce zero inbox rows while
            // the camera lane has already recorded AttentionRequired. Read only the
            // bounded operational flags needed for this projection; account IDs,
            // credentials and provider event identities stay server-side.
            var connector = await database.QuerySingleAsync("""
                SELECT
                  COALESCE(BOOL_OR(
                    status='Connected'
                    AND provider_account_ref IS NOT NULL
                    AND provider_account_verified_at IS NOT NULL),FALSE) provider_connection_observed,
                  COALESCE(BOOL_OR(
                    status='Error'
                    AND provider_account_ref IS NOT NULL),FALSE) provider_connection_attention,
                  COALESCE(BOOL_OR(
                    config_json->>'cameraSafetyStatus'='AttentionRequired'),FALSE) camera_attention,
                  COALESCE(BOOL_OR(
                    config_json->>'cameraSafetyStatus'='ProviderDataPendingVerification'),FALSE) camera_intake_observed
                FROM integrations
                WHERE company_id=@company AND integration_key='samsara'
                """, command => command.Parameters.AddWithValue("company", companyId), ct)
                ?? throw new InvalidOperationException("Camera connector status did not return a result.");
            ...
            var connected = Bool(connector, "providerConnectionObserved");
            var connectionAttention = Bool(connector, "providerConnectionAttention");
            var cameraAttention = Bool(connector, "cameraAttention");
            var cameraIntakeObserved = Bool(connector, "cameraIntakeObserved");
            var status = cameraAttention || quarantined > 0 && matched == 0 || connectionAttention && !connected
                ? "AttentionRequired"
                : observed > 0 || cameraIntakeObserved
                    ? "ProviderDataPendingVerification"
                    : connected
                        ? "AwaitingCameraIntake"
                        : "AwaitingProviderConnection";
