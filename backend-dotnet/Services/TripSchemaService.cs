...
public sealed class TripSchemaService(Database db)
{
    ...
    [
        ...
        "CREATE UNIQUE INDEX IF NOT EXISTS ux_trips_current_route ON trips(company_id, route_id) WHERE route_id IS NOT NULL AND LOWER(COALESCE(status,'')) IN ('planned','active','exception')",
