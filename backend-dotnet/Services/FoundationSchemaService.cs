...
public sealed class FoundationSchemaService(Database db)
{
    ...
    [
        ...
        "UPDATE ai_recommendations SET company_id=COALESCE(company_id, tenant_id), tenant_id=COALESCE(tenant_id, company_id) WHERE company_id IS NULL OR tenant_id IS NULL",
