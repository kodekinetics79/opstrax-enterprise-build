...
// Detention Recovery schema (consultant-signed spec, docs/DETENTION_RECOVERY_SPEC.md). All tables carry
// company_id BIGINT for RLS auto-enrollment (registered before RlsReconciliation). Idempotent.
public sealed class DetentionSchemaService(Database db)
{
    public async Task EnsureAsync()
    {
        // 1) Geofence -> billable party (a fence opts into detention by carrying a customer + site_role).
        await db.ExecuteAsync("ALTER TABLE geofences ADD COLUMN IF NOT EXISTS customer_id BIGINT NULL");
        await db.ExecuteAsync("ALTER TABLE geofences ADD COLUMN IF NOT EXISTS site_role VARCHAR(30) NULL");
        await db.ExecuteAsync("CREATE INDEX IF NOT EXISTS idx_geofences_company_customer ON geofences (company_id, customer_id) WHERE customer_id IS NOT NULL");

        // 2) Shipper references on the load (the AP clerk's 2-minute match requirement).
        await db.ExecuteAsync("ALTER TABLE jobs ADD COLUMN IF NOT EXISTS po_number VARCHAR(80) NULL");
        await db.ExecuteAsync("ALTER TABLE jobs ADD COLUMN IF NOT EXISTS bol_number VARCHAR(80) NULL");
        await db.ExecuteAsync("ALTER TABLE jobs ADD COLUMN IF NOT EXISTS rate_con_number VARCHAR(80) NULL");
        await db.ExecuteAsync("ALTER TABLE jobs ADD COLUMN IF NOT EXISTS appointment_ref VARCHAR(80) NULL");

        // 3) Rule cards — fail-closed pricing config (no card => detect + show, price NOTHING).
        ...
        await db.ExecuteAsync("""
            ...
            """);
        await db.ExecuteAsync("CREATE UNIQUE INDEX IF NOT EXISTS uq_detention_dwell_entry ON detention_dwells (company_id, entry_event_id)");
        await db.ExecuteAsync("CREATE UNIQUE INDEX IF NOT EXISTS uq_detention_dwell_open ON detention_dwells (company_id, geofence_id, vehicle_id) WHERE status='open'");
        await db.ExecuteAsync("CREATE INDEX IF NOT EXISTS idx_detention_dwells_company_status ON detention_dwells (company_id, status, entered_at DESC)");

        // 5) Consumed-event ledger — every geofence event a dwell absorbs gets exactly one row, ever.
        // (Fixes both consultant blockers: no unconsumed re-entry can seed a duplicate dwell, and a
        // superseded Exit can never re-close a merged dwell.)
        await db.ExecuteAsync("""
            ...
            """);
        await db.ExecuteAsync("CREATE UNIQUE INDEX IF NOT EXISTS uq_detention_event_consumed ON detention_dwell_events (company_id, geofence_event_id)");
        await db.ExecuteAsync("CREATE INDEX IF NOT EXISTS idx_detention_dwell_events_dwell ON detention_dwell_events (company_id, dwell_id, event_time)");

        // 6) Notice log (contemporaneous-notice proof — welded into evidence).
        await db.ExecuteAsync("""
            ...
            """);
        await db.ExecuteAsync("CREATE UNIQUE INDEX IF NOT EXISTS uq_detention_notice ON detention_notices (company_id, dwell_id, notice_type)");

        // 7) Evidence — frozen at pricing, immutable by trigger (only share fields updatable).
        await db.ExecuteAsync("""
            ...
            """);
        await db.ExecuteAsync("CREATE UNIQUE INDEX IF NOT EXISTS uq_detention_evidence_dwell ON detention_evidence (company_id, dwell_id)");
        await db.ExecuteAsync("CREATE UNIQUE INDEX IF NOT EXISTS uq_detention_evidence_token ON detention_evidence (share_token) WHERE share_token IS NOT NULL");
        await db.ExecuteAsync("""
            ...
            $$ LANGUAGE plpgsql
            """);
        await db.ExecuteAsync("DROP TRIGGER IF EXISTS trg_detention_evidence_immutable ON detention_evidence");
        await db.ExecuteAsync("CREATE TRIGGER trg_detention_evidence_immutable BEFORE UPDATE OR DELETE ON detention_evidence FOR EACH ROW EXECUTE FUNCTION detention_evidence_immutable()");

        // 8) Weld to the money spine: one charge per dwell, ever.
