...
public sealed class DispatchPilotPostgresTests
{
    ...
    private static async Task<SeedData> Seed(Database db)
    {
        ...
        await db.ExecuteAsync(
            ...
            c => { c.Parameters.AddWithValue("@c", company); c.Parameters.AddWithValue("@b", branch); c.Parameters.AddWithValue("@d", driver); c.Parameters.AddWithValue("@event", $"dispatch-{suffix}"); });
    ...
    {
        foreach (var sql in new[] { "DELETE FROM dispatch_proof_uploads WHERE company_id=@c", "DELETE FROM dispatch_proof_artifacts WHERE company_id=@c", "DELETE FROM dispatch_proofs WHERE company_id=@c", "DELETE FROM dispatch_exceptions WHERE company_id=@c", "DELETE FROM audit_logs WHERE company_id=@c", "DELETE FROM outbox_messages WHERE tenant_id=@c", "DELETE FROM dispatch_assignments WHERE company_id=@c", "DELETE FROM trips WHERE company_id=@c", "DELETE FROM route_stops WHERE route_id IN (SELECT id FROM routes WHERE company_id=@c)", "DELETE FROM routes WHERE company_id=@c", "DELETE FROM jobs WHERE company_id=@c", "DELETE FROM vehicles WHERE company_id=@c", "DELETE FROM hos_clocks WHERE company_id=@c", "DELETE FROM drivers WHERE company_id=@c", "DELETE FROM users WHERE company_id=@c", "DELETE FROM branches WHERE company_id=@c", "DELETE FROM companies WHERE id=@c" })
