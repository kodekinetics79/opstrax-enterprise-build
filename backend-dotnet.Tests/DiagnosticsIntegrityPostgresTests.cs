...
public sealed class DiagnosticsIntegrityPostgresTests
{
    ...
    public async Task ConcurrentAlteredPayload_WithSameSourceEvent_AcceptsOneAtomicBatch()
    {
        ...
        try
        {
            var a = AcceptBatchAsync(CreateDatabase(), company, device, source, ["J1939:ENGINE:SPN:10:FMI:1", "J1939:ENGINE:SPN:11:FMI:2"]);
            var b = AcceptBatchAsync(CreateDatabase(), company, device, source, ["J1939:ENGINE:SPN:99:FMI:9", "J1939:ENGINE:SPN:98:FMI:8"]);
    ...
    private static Task<bool> AcceptBatchAsync(Database db, long company, string device, string source, string[] identities) =>
        db.WithTransactionAsync(async (connection, transaction) =>
        {
            ...
            await using (var exists = new Npgsql.NpgsqlCommand(
                "SELECT EXISTS(SELECT 1 FROM fault_occurrences WHERE company_id=@c AND device_id=@d AND source_event_id=@s)", connection, transaction))
            {
                exists.Parameters.AddWithValue("@c", company);
                exists.Parameters.AddWithValue("@d", device);
                exists.Parameters.AddWithValue("@s", source);
                if (Convert.ToBoolean(await exists.ExecuteScalarAsync())) return false;
            }
            for (var ordinal = 0; ordinal < identities.Length; ordinal++)
            {
                await using var insert = new Npgsql.NpgsqlCommand(
                    @"INSERT INTO fault_occurrences
                        (company_id,device_id,source_event_id,dtc_ordinal,canonical_dtc,observed_at,protocol,code,spn,fmi)
                      VALUES (@c,@d,@s,@o,@i,NOW(),'J1939',@i,@o,@o)", connection, transaction);
