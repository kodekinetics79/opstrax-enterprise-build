...
public sealed class SafetyIncidentsPilotRegressionTests
{
    ...
    public async Task IncidentReferences_RejectMixedDirectAndEventAssetBranches()
    {
        ...
        try
        {
            ...
            var vehicleA = await db.InsertAsync("INSERT INTO vehicles(company_id,branch_id,vehicle_code,type,status) VALUES(@c,@b,@code,'Truck','Available')", c => { c.Parameters.AddWithValue("@c", companyId); c.Parameters.AddWithValue("@b", branchA); c.Parameters.AddWithValue("@code", $"VEH-A-{companyId}"); });
            var vehicleB = await db.InsertAsync("INSERT INTO vehicles(company_id,branch_id,vehicle_code,type,status) VALUES(@c,@b,@code,'Truck','Available')", c => { c.Parameters.AddWithValue("@c", companyId); c.Parameters.AddWithValue("@b", branchB); c.Parameters.AddWithValue("@code", $"VEH-B-{companyId}"); });
