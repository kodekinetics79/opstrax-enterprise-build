    ...
    private sealed class Fixture : IAsyncDisposable
    {
        ...
        internal static async Task<Fixture> Create()
        {
            ...
            try
            {
                ...
                f.CompanyId = await f.Db.InsertAsync("INSERT INTO companies(company_code,name,industry,country) VALUES(@code,'Synthetic partial GPS','Transportation','US') RETURNING id", c => c.Parameters.AddWithValue("@code", $"SPG-{suffix}"));
                ...
                var branchId = await f.Db.InsertAsync("INSERT INTO branches(company_id,branch_code,name,status,country_code) VALUES(@cid,@code,'Synthetic branch','Active','US') RETURNING id", c => { c.Parameters.AddWithValue("@cid", f.CompanyId); c.Parameters.AddWithValue("@code", $"SPG-B-{suffix}"); });
