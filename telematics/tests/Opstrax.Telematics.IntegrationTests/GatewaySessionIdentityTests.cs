...
public class GatewaySessionIdentityTests
{
    ...
    // ── Fix continuity reaches the published event ─────────────────────────────

    /// <summary>
    /// End to end: a fix that could not physically follow the previous one arrives on the backbone
    /// FLAGGED, and still arrives. The quality flag has been part of the canonical contract and
    /// persisted by the projection since both were written, with nothing ever setting it; this is
    /// the test that says it is now produced.
    /// </summary>
    [Fact]
    public async Task An_impossible_displacement_is_published_flagged_and_is_not_dropped()
    {
        await using GatewayHarness gw = await GatewayHarness.StartAsync();
        IEventSubscription<CanonicalTelemetryEvent> delivered =
            gw.Bus.Subscribe<CanonicalTelemetryEvent>(TelematicsTopics.TelemetryNormalized);
        ...
        // Washington DC, then Tokyo sixty seconds later.
        await stream.WriteAsync(BuildLocationFrame(2, BaseFix, lat: 38.8951, lng: 77.0364));
        CanonicalTelemetryEvent first = await NextEventAsync(delivered);
        Assert.False(first.Quality.TeleportSuspected);
        Assert.True(first.Quality.IsClean);

        await stream.WriteAsync(BuildLocationFrame(3, BaseFix.AddMinutes(1), lat: 35.6762, lng: 139.6503));
        CanonicalTelemetryEvent jump = await NextEventAsync(delivered);

        // Flagged...
        Assert.True(jump.Quality.TeleportSuspected);
        Assert.False(jump.Quality.IsClean);
        // ...and NOT dropped. A plausibility heuristic must never be the thing that loses a fix.
        Assert.Equal(DeviceA, jump.DeviceId);
        Assert.NotNull(jump.Location);
        Assert.Equal(35.6762, jump.Location!.Value.Lat, 3);

        await WaitUntilAsync(() => gw.Metrics.TeleportsSuspected == 1, SocketTimeout);
        Assert.Equal(1, gw.Metrics.TeleportsSuspected);
    }

    /// <summary>Ordinary driving publishes clean, so the flag above is not simply always on.</summary>
    [Fact]
    public async Task Ordinary_movement_is_published_clean()
    {
        await using GatewayHarness gw = await GatewayHarness.StartAsync();
        IEventSubscription<CanonicalTelemetryEvent> delivered =
            gw.Bus.Subscribe<CanonicalTelemetryEvent>(TelematicsTopics.TelemetryNormalized);
        ...
        // Six minutes of driving north at roughly 90 km/h.
        await stream.WriteAsync(BuildLocationFrame(2, BaseFix, lat: 38.0000, lng: 77.0));
        Assert.True((await NextEventAsync(delivered)).Quality.IsClean);

        await stream.WriteAsync(BuildLocationFrame(3, BaseFix.AddMinutes(6), lat: 38.0810, lng: 77.0));
        CanonicalTelemetryEvent moved = await NextEventAsync(delivered);

        Assert.True(moved.Quality.IsClean);
        Assert.Equal(0, gw.Metrics.TeleportsSuspected);
    }


    /// <summary>
    /// The exported series must agree with the in-process counter, for every signal an operator is
    /// expected to alert on.
    /// </summary>
    /// <remarks>
    /// <para>
    /// These are counted twice: once into <c>GatewayMetrics</c>, which is process-local and which
    /// nothing scrapes, and once into an OpenTelemetry instrument, which is what actually reaches
    /// Prometheus. Two counters for one event is a drift waiting to happen, and a drift here is
    /// silent: the dashboard keeps drawing a flat green line for a condition that is occurring.
    /// </para>
    /// <para>
    /// So the agreement is asserted rather than assumed. That is the same discipline that was
    /// missing when this decoder's fixtures were generated by the decoder they were checking.
    /// </para>
    /// </remarks>
    [Fact]
    public async Task Exported_metrics_agree_with_the_in_process_counters()
    {
        _ = TelematicsInstrumentation.Meter;

        var exported = new ConcurrentDictionary<string, long>();
        ...
        listener.SetMeasurementEventCallback<long>((instrument, value, _, _) =>
            exported.AddOrUpdate(instrument.Name, value, (_, running) => running + value));
        listener.Start();
        ...
        // One good fix, one corrupt frame, then an impossible displacement: exercises the CRC,
        // teleport and frame-received series in a single connection.
        await stream.WriteAsync(BuildLocationFrame(2, BaseFix, lat: 38.8951, lng: 77.0364));
        await NextEventAsync(delivered);

        byte[] corrupt = BuildLocationFrame(3, BaseFix.AddSeconds(10), lat: 38.9, lng: 77.0);
        corrupt[^3] ^= 0xFF;
        await stream.WriteAsync(corrupt);

        await stream.WriteAsync(BuildLocationFrame(4, BaseFix.AddMinutes(1), lat: 35.6762, lng: 139.6503));
        await NextEventAsync(delivered);

        // A second connection for the same device, to move the displacement counter.
        using TcpClient second = await gw.ConnectAsync();
        NetworkStream secondStream = second.GetStream();
        byte[] secondLogin = BuildLoginFrame(ImeiA, serial: 1);
        await secondStream.WriteAsync(secondLogin);
        Assert.Equal(AckForFrame(secondLogin), await ReadExactlyAsync(secondStream, 10));

        await WaitUntilAsync(
            () => gw.Metrics.CrcFailures == 1
               && gw.Metrics.TeleportsSuspected == 1
               && gw.Metrics.DuplicateSessionsDisplaced == 1,
            SocketTimeout);
        listener.RecordObservableInstruments();

        long Exported(string name) => exported.TryGetValue(name, out long v) ? v : 0;

        Assert.Equal(gw.Metrics.CrcFailures, Exported("opstrax_telematics_crc_failures"));
        Assert.Equal(gw.Metrics.FramesReceived, Exported("opstrax_telematics_frames_received"));
        Assert.Equal(gw.Metrics.TeleportsSuspected, Exported("opstrax_telematics_teleport_suspected"));
        Assert.Equal(
            gw.Metrics.DuplicateSessionsDisplaced,
            Exported("opstrax_telematics_duplicate_sessions_displaced"));

        // And the values are the ones the scenario actually produced, so an agreement of two zeroes
        // cannot pass this test.
        Assert.Equal(1, gw.Metrics.CrcFailures);
        Assert.Equal(1, gw.Metrics.TeleportsSuspected);
        Assert.Equal(1, gw.Metrics.DuplicateSessionsDisplaced);
        Assert.True(gw.Metrics.FramesReceived >= 5);
    }
