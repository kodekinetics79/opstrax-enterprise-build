...
| STG-011 | The protected migration runner omitted nine pre-RLS foundation migrations. The first real API boot consequently reported ready while `outbox_messages` was absent and the outbox worker failed every five seconds. | The runner now reconciliation-applies the nine idempotent foundation stages before later migrations, requires their ledgers and verifies outbox/inbox at the owner post-check. The clean-chain and CI contracts enforce the order. The isolated DB now has all nine ledgers and both empty queue tables; the terminal Stage 58/59/67/76 chain was reapplied afterward. | FIXED in source and isolated DB; corrected API redeploy pending |
| STG-012 | Protected runtime schema initialization is correctly disabled, but Stage 26's `platform_admins` table lacked the invite, update and MFA columns used by the bootstrap reconciler and platform endpoints. The bootstrap failed, leaving zero platform admins. Existing readiness did not cover these columns. | Stage 77 now adds the four columns idempotently and the readiness contract requires the table and exact column shapes. The isolated DB has all four columns; it still intentionally has zero admins until the corrected API is redeployed with the one-time reset armed. | FIXED in source and isolated DB; bootstrap/audit/login evidence pending redeploy |
| STG-013 | Stage 18 assumed the modern `contracts.contract_number` schema, but the baseline can contain the legacy `contract_code`/`expiration_date` shape from migration 001. Reconciliation failed at the Stage 18 index. | Stage 18 now additively reconciles the modern contract columns, backfills compatible legacy values when present and only then enforces the required key. Two real runner passes reached terminal security reconciliation after the repair. | FIXED in source and isolated DB; clean-chain CI pending |
| STG-014 | Exact-head CI run 31652471023 proved that Stage 5 could not reconcile the migration-001 `ai_recommendations` predecessor: migration 001 uses `company_id` and has no `tenant_id`/`created_at`, while Stage 5 created a tenant-id index before any compatibility repair. | Stage 5 now installs both scope columns, backfills them bidirectionally, requires non-null `tenant_id`, and adds the missing timestamp before indexes. The clean-chain and source contracts assert the reconciled shape. | FIXED in source; exact-head clean-chain CI pending |
| STG-015 | After STG-014, exact-head CI run 31652637133 advanced to Stage 6 and proved that migration 001 creates `trips` but not `trip_stops`; Stage 6 altered/indexed the missing table before Stage 51 could create it. Both DB-backed CI jobs failed closed. | Stage 6 now creates the same canonical `trip_stops` shape used by Stage 51 before applying its additive address/index contract. A source-order regression prevents the dependency from returning. | FIXED in source; exact-head DB CI pending |
| STG-016 | Exact-head CI run 31653107764 reached the DB-backed runtime suites and proved that Stage 18 made `contract_number` mandatory while revenue tests and the demo seeder still write the migration-001 `contract_code/title/expiration_date` shape. The authenticated contracts API has the inverse defect: it writes only the modern fields while the legacy columns remain mandatory. Thirteen integration tests failed closed. | Stage 18 now installs an idempotent database-boundary compatibility trigger that synchronizes contract code/number and expiry names in both directions and supplies an evidence-backed display title for the modern writer. The clean-chain executes legacy/modern creates and updates instead of relying on source inspection. A disposable PostgreSQL 17 run passed the full clean chain and all 340 DB-backed integration tests. | FIXED in source and local executable regression; exact-head DB CI pending |
| STG-017 | The first authenticated tenant-provisioning request against exact SHA `965b5df4f89f8bc630cffe8554df5cc9d1277f0e` failed HTTP 500 with trace `a93b1dd57ba431eef09af1fbc6896125`: protected Staging skips runtime schema initialization, but the owner migration chain omitted `country_profiles`, which `TenantCreate` reads before any tenant insert. No tenant was created. | Stage 78 owner-migrates the country-profile table, company cascade columns and 24 localization reference profiles. Runner/clean-chain checks require the ledger and US/CA/SA catalog truth; protected readiness now fails closed when the table, reference rows or ledger are absent; both API images and release provenance must carry the migration. Stage 78 is applied to isolated Neon and exact SHA `d0f6c2f8189a85935431c7bf3529058f66bd99a5` was deployed to both Render and Vercel. | FIXED and DEPLOYED; follow-on provisioning contract defect is STG-018 |
| STG-018 | Retrying the authenticated tenant-provisioning request against exact SHA `d0f6c2f8189a85935431c7bf3529058f66bd99a5` failed HTTP 500 with trace `2daa00090351fac340331d3a6c311331` and correlation/reference `9ea14a87402c4e2c95c25171f673d1d1`: PostgreSQL reported missing `companies.legal_name`. A dependency inventory then proved that all eight extended company fields, `tenant_subscriptions.billing_cycle`, `password_reset_tokens` and `feature_flags` were absent from the protected owner chain. The system request transaction rolled back; database evidence showed zero `CERT-SYN-ALPHA` companies after failure. | Stage 79 owner-migrates every `TenantCreate` schema dependency, rejects ambiguous cross-tenant email ownership, adds a case-insensitive per-tenant email key without introducing legacy-GUC policies, and relies on the runner's terminal Stage58 reconciliation for exact RLS/ACLs. Readiness now requires both new tenant tables, all write-path columns, the Stage79 ledger and the email key, and exposes `tenant_provisioning_ready`. The real PostgreSQL 17 clean-chain passed two applications plus deliberate Stage79 table/column loss and repair; focused migration/provenance/readiness regressions pass 5/5. | FIXED in source and executable clean-chain; exact-head CI/migration/redeploy pending |

### Runtime migration-repair evidence

- The first API deployment at `dffdf1b8bd4a6935080b2c8307ce6bc475479e75` exposed STG-011 and STG-012 through structured Render logs. Readiness was therefore a false positive and that deployment is not certified.
- The repaired runner positively targeted only Neon project `wild-paper-65837531`, database `opstrax_staging`. Production was not connected, inspected or modified.
- Post-repair counts: terminal Stage 58/59/67/76 ledgers `4/4`; required foundation ledgers `9/9`; required outbox/inbox/platform tables `3/3`; required platform-admin columns `4/4`; companies `0`; platform admins `0`; outbox rows `0`; inbox rows `0`.
- The migration runner completed its terminal message `Existing Stage58/59/67/76 deployment reconciled with Stage76 terminal.` No data seed was run.
- A new exact candidate must pass CI, be deployed to both Render and Vercel, report the same SHA, create exactly one audited bootstrap operator, and show healthy workers with no recurring outbox error before tenant/persona creation begins.
