...
The tenant fault-code API and maintenance/diagnostics pages expose the protocol, source address, CAN channel, observation time, evidence classification, bounded capture references, the complete capture-set digest and explicit safety-action status. A row receives `CanonicalCanObservation` only when its company, registry device, vehicle, observation time, immutable envelope event identifier and DTC identity all match a persisted canonical diagnostic event. `ObservationOnly` is rendered as a review requirement rather than an automatic hold. The API does not return unrestricted `raw_evidence` JSON, and the maintenance insight text does not claim that a defect or hold exists unless the persisted hold join proves it.

Persisted diagnostic holds now have a dedicated customer-visible Maintenance tab. Authorized operators can acknowledge a hold without changing vehicle availability, then resolve it only with a typed technician-scan, provider-diagnostic or service-record reference plus resolution notes. The existing server transaction records the verification and re-evaluates every remaining availability blocker before releasing a vehicle.
