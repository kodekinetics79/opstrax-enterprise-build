...
This slice establishes durable canonical diagnostic evidence. It does not yet project DM1 into maintenance `fault_codes` or `diagnostic_holds`, and it does not infer that a missing code or DM2 record cleared an active fault. That maintenance consumer is a separate engineering gate because it can ground vehicles and create work orders.
