...
| Provider handshake | Replacement hardening requires successful read-only calls to `GET /me`, `GET /fleet/vehicles?limit=1` and the bounded vehicle statistics feed before setting `Connected`; partial-scope and malformed-envelope responses fail closed. The server-only organization identity returned by `/me` binds discovered devices and telemetry idempotency to the real provider account. | CODE READY / FIELD BLOCKED | New exact-SHA hosted proof plus authorized Samsara admin/account/token, provider responses and all three-scope evidence |
| Provider cloud region | Connector configuration strictly selects the official US, EU/UK or Canada API host; arbitrary URLs are rejected before provider I/O. A region change clears provider identity plus GPS/camera cursors and requires a new handshake. | CODE READY / FIELD BLOCKED | Authorized provider account and end-to-end field evidence for every region claimed in the release candidate |
...
- The follow-up candidate corrects an automatic-worker regression introduced by provider-account binding: unattended GPS sync now carries the verified provider organization reference in the generation-bound operation body. A serialization-level regression test proves the worker sends the tenant, integration, generation, lease, provider account and durable cursor together. This requires hosted and deployed verification before it is treated as production evidence.
