...
19. Stage 125 adds an exact-device spare-pool plan with append-only `Added → Reserved → Released → Removed` history. A reservation is serialized against one open, in-scope RMA case for a different failed device, and the database prevents two active spares from being reserved to the same case. An installed device cannot enter the plan, and an active pool member cannot be retired until it is removed. The customer surface shows current availability and reservation history while every record remains `OperatorRecordedUnverified`; physical possession, condition, compatibility, installation and certification stay false.
20. Stage 126 separates device support routing from connectivity and certification. Operators can assign, materially change, end and later reassign an exact device tier, coverage window, routing response target, escalation policy and commercial reference. The append-only state machine is tenant/branch scoped, serialized per device and idempotent. Every customer-visible record stays `OperatorRecordedUnverified`; it cannot assert that a contract grants entitlement, a provider supports the device, the hardware is supportable, or the device is certified.
...
| Spare-device pool | Exact-device membership, availability, one-case reservation, release and removal history implemented with database serialization and customer controls | OperatorRecordedUnverified planning only; possession, condition, compatibility, installation and certification remain unverified |
| Device support tier | Exact-device assign/change/end history, routing target, coverage window, escalation and commercial references implemented with customer controls | OperatorRecordedUnverified routing only; commercial entitlement, provider support, hardware supportability and certification remain unverified |
