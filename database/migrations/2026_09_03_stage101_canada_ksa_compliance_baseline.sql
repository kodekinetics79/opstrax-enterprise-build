-- Stage 101 — Canada + Saudi Arabia regulatory baseline hardening
--
-- Purpose
...
--
-- Apply as database owner after Stage 100.
...
BEGIN
  IF to_regclass('public.countries') IS NULL
    ...
    RAISE EXCEPTION 'Stage101 requires countries, compliance_profiles and compliance_rules';
  END IF;
END
$preflight$;
...
BEGIN
  ...
  IF p IS NULL THEN
    RAISE EXCEPTION 'Stage101 could not resolve Canada south-of-60 compliance profile';
  END IF;
...
BEGIN
  ...
  IF p IS NULL THEN
    RAISE EXCEPTION 'Stage101 could not resolve Canada north-of-60 compliance profile';
  END IF;
...
BEGIN
  ...
  IF p IS NULL THEN
    RAISE EXCEPTION 'Stage101 could not resolve Saudi TGA goods-transport profile';
  END IF;
...
BEGIN
  IF EXISTS (SELECT 1 FROM countries WHERE code='SA' AND hos_ruleset ILIKE '%SASO%') THEN
    RAISE EXCEPTION 'Stage101 failed: Saudi country ruleset still references SASO HOS';
  END IF;

  IF EXISTS (SELECT 1 FROM compliance_profiles WHERE country_code='SA' AND authority ILIKE '%SASO%') THEN
    RAISE EXCEPTION 'Stage101 failed: Saudi compliance profile still attributes HOS authority to SASO';
  END IF;

  IF EXISTS (SELECT 1 FROM compliance_rules WHERE rule_code='SA-HOS-10H' AND is_active) THEN
    RAISE EXCEPTION 'Stage101 failed: obsolete SA-HOS-10H rule remains active';
  END IF;

  IF EXISTS (SELECT 1 FROM compliance_rules WHERE rule_code='TC-NSC-CARRIER' AND is_active) THEN
    RAISE EXCEPTION 'Stage101 failed: obsolete generic TC-NSC-CARRIER rule remains active';
  END IF;
  ...
  IF missing_count <> 0 THEN
    RAISE EXCEPTION 'Stage101 failed: % mandatory Canada/KSA baseline rules missing', missing_count;
  END IF;
END
$postcondition$;
