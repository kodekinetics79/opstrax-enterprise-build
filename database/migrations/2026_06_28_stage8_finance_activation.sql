-- Stage 8 Finance Activation Schema Contract
-- Additive, tenant-scoped, local-review migration artifact.
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_issued_invoices_company_invoice_no
    ON issued_invoices (company_id, invoice_number);

CREATE UNIQUE INDEX IF NOT EXISTS uq_issued_invoices_company_source_draft
    ON issued_invoices (company_id, source_invoice_draft_id);

CREATE INDEX IF NOT EXISTS idx_issued_invoices_company_status
    ON issued_invoices (company_id, status, issued_at DESC);

CREATE INDEX IF NOT EXISTS idx_issued_invoices_company_customer
    ON issued_invoices (company_id, customer_id, issued_at DESC);

CREATE INDEX IF NOT EXISTS idx_issued_invoices_company_due_at
    ON issued_invoices (company_id, due_at);

CREATE INDEX IF NOT EXISTS idx_issued_invoice_lines_company_invoice
    ON issued_invoice_lines (company_id, issued_invoice_id, line_no);

CREATE INDEX IF NOT EXISTS idx_invoice_payments_company_invoice
    ON invoice_payments (company_id, issued_invoice_id, received_at DESC);

CREATE INDEX IF NOT EXISTS idx_invoice_payments_company_reference
    ON invoice_payments (company_id, payment_reference);
