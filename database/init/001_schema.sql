...
-- Core Fleet organizational scope must exist on a clean bootstrap. This used to
-- live only in a dated migration, while the runtime API always queried branches
-- and branch_id, so fresh databases failed as soon as Core Fleet opened.
CREATE TABLE IF NOT EXISTS branches (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  company_id BIGINT NOT NULL,
  branch_code VARCHAR(64) NOT NULL,
  name VARCHAR(200) NOT NULL,
  branch_type VARCHAR(32) NOT NULL DEFAULT 'branch',
  region VARCHAR(120),
  address VARCHAR(300),
  city VARCHAR(120),
  state VARCHAR(120),
...
CREATE TABLE IF NOT EXISTS drivers (
  ...
  license_number_bidx VARCHAR(64) NULL,
...
ALTER TABLE vehicle_assignments ADD COLUMN IF NOT EXISTS branch_id BIGINT NULL;
ALTER TABLE vehicle_assignments ADD COLUMN IF NOT EXISTS released_at TIMESTAMPTZ NULL;
WITH ranked AS (
...
UPDATE vehicle_assignments va SET status='Released', released_at=COALESCE(va.released_at,NOW())
FROM ranked r WHERE va.id=r.id AND r.rn>1;
WITH ranked AS (
...
ALTER TABLE jobs ADD COLUMN IF NOT EXISTS branch_id BIGINT NULL;
UPDATE jobs j SET branch_id=v.branch_id FROM vehicles v
WHERE j.branch_id IS NULL AND j.assigned_vehicle_id=v.id AND j.company_id=v.company_id AND v.branch_id IS NOT NULL;
UPDATE jobs j SET branch_id=d.branch_id FROM drivers d
WHERE j.branch_id IS NULL AND j.assigned_driver_id=d.id AND j.company_id=d.company_id AND d.branch_id IS NOT NULL;
