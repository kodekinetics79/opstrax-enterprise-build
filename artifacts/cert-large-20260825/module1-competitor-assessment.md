...
Latest deployed OpsTrax candidate exercised: `b1313ed5b32ee12e5cbcc625e397b0bc38037e0a`
...
1. The 1,000-vehicle journey is substantially evidenced and 500 drivers are persisted/exported, but the complete 1,250-driver / 1,100-device / 300-asset import and reconciliation remains open.
2. No browser proof that invalid and duplicate identities are rejected without partial corruption.
3. Vehicle/driver effective-dated reassignment history now has preserved Chrome evidence; device and trailer relationships remain open.
4. Archive/reactivate worked in a b131 Chrome observation but its screenshot bytes were not preserved, so objective lifecycle proof must be repeated.
...
| Assignment and history | Samsara exposes time-ranged driver-vehicle assignment records and supports bulk static assignments. Fleetio presents current, past, and future assignments, start/end times, reassignment, and assignment history; it rejects overlapping open-ended assignments. | One governed vehicle/driver reassignment has preserved effective-from/effective-to history through refresh and logout/login. Device and trailer assignment histories remain open. | Partial parity demonstrated for vehicle/driver; full Module 1 relationship coverage remains below the bar. |
...
| Large-list usability | Fleetio import history is searchable/filterable and exposes status/count columns; its asset/contact lists support searching, filtering, sorting, and group filtering. Geotab supports group filters and paginated/sorted device retrieval. Samsara APIs use cursor pagination. | The 1,000-vehicle total and boundary searches are preserved. A 500-driver roster paged 100 rows at a time and exported 500 records, but its screenshots were not retained. The corrective candidate adds server paging/export for 300 assets and 1,100 devices. | Vehicle volume is partly demonstrated; corrective-SHA Chrome timings, assets/devices, and all four viewports remain required. |
