...
assert.match(controlTower, /canViewDeviceEvidence \? \[\["Online devices"/, "Online-device evidence must be hidden without device permission");
...
assert.match(alertsCenter, /persisted telemetry alert records/i, "Alerts Center must identify its persisted operational source");
...
assert.match(alertsCenter, /const visibleRows = filtered\.slice/, "Alerts pagination must consume the complete status-filtered record set");
assert.match(alertsCenter, /visibleRows\.map\(alert =>/, "Alerts must render the current filtered page, including resolved records when selected");
assert.doesNotMatch(
  alertsCenter,
