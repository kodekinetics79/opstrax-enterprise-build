...
assert.match(page, /requiredViewPermission: PERMISSIONS\.TELEMATICS_DIAGNOSTICS_VIEW/, "diagnostics declares its direct read boundary");
assert.match(page, /const canView = hasDirectPermission\(config\.requiredViewPermission\)/, "the page evaluates the same direct permission enforced by the API");
