...
assert.match(moduleConfig, /key: "dashcam",\s+title: "Camera Safety",\s+route: "\/dashcam"/, "the camera safety route is visible under its customer-facing capability name");
assert.match(moduleConfig, /Camera provider intake status, events awaiting verification, and stored manual metadata\./, "navigation copy describes the implemented provider intake and manual record surfaces");
assert.match(moduleConfig, /requiredPermission: "safety:evidence:view"/, "camera navigation uses the same evidence permission as its route");
