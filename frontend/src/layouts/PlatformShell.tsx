...
export function PlatformShell() {
  ...
  const visibleNav = NAV.filter((item) => can(item.permission)
    && (item.to !== "/platform/product-pilot" || session?.productPilotAvailable === true));
  ...
  return (
    <div className="platform-shell flex min-h-screen text-slate-100">
      {/* Sidebar */}
      <aside className="glass-nav-dark hidden w-60 shrink-0 flex-col border-r px-3 py-4 shadow-[0_14px_40px_rgba(2,6,23,.3)] lg:flex">
        <div className="flex items-center gap-2.5 rounded-xl border border-slate-800/80 bg-white/5 px-3 py-2">
          <OpsTraxLogo size={28} />
          <div>
        ...
        <nav className="mt-4 flex flex-1 flex-col gap-0.5">
          {visibleNav.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex min-h-8 items-center gap-2.5 rounded-lg px-3 py-1.5 text-[13px] font-medium transition ${
                  isActive
        ...
        <div className="mt-auto rounded-xl border border-slate-800/80 bg-slate-900/70 p-3">
          ...
          <button
            ...
            className="mt-3 flex min-h-8 w-full items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-1.5 text-xs font-semibold text-slate-200 transition hover:border-red-500/40 hover:text-red-300"
          >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="platform-workspace flex-1 overflow-y-auto px-4 py-4 lg:px-6 lg:py-5">
