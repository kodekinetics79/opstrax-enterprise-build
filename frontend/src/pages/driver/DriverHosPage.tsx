...
function DailyCertificationPanel() {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
    <div><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Daily records</p><p className="mt-1 text-xs text-slate-500">Driver attestation records your review in OpsTrax. It is not ELD product certification and does not submit data to a regulator.</p></div>
    {byDay.size === 0 ? <p className="text-sm text-slate-500">No persisted HOS duty-status records are available.</p> : [...byDay.entries()].map(([date, rows]) => {
      ...
      return <div key={date} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 p-3"><div><p className="text-sm font-semibold text-slate-800">{date ? new Date(`${date}T12:00:00`).toLocaleDateString() : "Unknown date"}</p><p className="text-xs text-slate-500">{rows.length} duty-status segment{rows.length === 1 ? "" : "s"}</p></div>{certified ? <span className="flex items-center gap-1 text-xs font-semibold text-emerald-700"><CheckCircle className="h-4 w-4" />Driver attested</span> : <button type="button" className="rounded-xl bg-teal-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-40" disabled={invalid} title={invalid ? "Close or correct all segments before attesting" : "Review and attest to this complete day"} onClick={() => { setAccepted(false); setSelected({ id: Number(rows[0].id), date }); }}>Review &amp; attest</button>}</div>;
    })}
    {certify.isError && <p className="text-sm text-red-700" role="alert">{(certify.error as Error)?.message}</p>}
    {selected && <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-label="Attest to daily HOS record"><div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl space-y-4"><div><h2 className="font-bold text-slate-900">Attest to {selected.date}</h2><p className="mt-1 text-sm text-slate-600">Review the complete daily record before accepting this attestation.</p></div><label className="flex items-start gap-3 rounded-xl border border-slate-200 p-3"><input type="checkbox" className="mt-1 h-4 w-4" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} /><span className="text-sm font-medium text-slate-800">{HOS_ATTESTATION}</span></label><div className="flex gap-2"><button type="button" className="flex-1 rounded-xl border border-slate-200 py-3 text-sm font-bold text-slate-600" onClick={() => setSelected(null)}>Cancel</button><button type="button" className="flex-1 rounded-xl bg-teal-600 py-3 text-sm font-bold text-white disabled:opacity-40" disabled={!accepted || certify.isPending} onClick={() => { void certifySingleFlight(() => certify.mutateAsync(selected.id)); }}>{certify.isPending ? "Recording…" : "Record attestation"}</button></div></div></div>}
  </section>;
}
