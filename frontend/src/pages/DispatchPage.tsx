...
export function DispatchPage() {
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow="Dispatch Board" title="AI-assisted assignment cockpit" description="Kanban dispatch with match scores, exception radar, SLA watch, customer ETA actions and status movement buttons." actions={<><button className="btn-primary" onClick={() => autoSuggest.mutate()}><Sparkles className="h-4 w-4" /> Auto Suggest</button><button className="btn-ghost" onClick={() => eta.mutate()}><Send className="h-4 w-4" /> Send ETA Updates</button><button className="btn-ghost" onClick={() => { const all = stages.flatMap(st => board.data?.[st] || []); const cols = ["jobNumber","customerName","status","driverName","vehicleCode","eta","slaStatus","priority"]; const csv = [cols.join(","), ...all.map(r => cols.map(c => JSON.stringify(r[c] ?? "")).join(","))].join("\n"); const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([csv], { type: "text/csv" })); a.download = "dispatch-plan.csv"; a.click(); }}><Download className="h-4 w-4" /> Export Dispatch Plan</button></>} />
