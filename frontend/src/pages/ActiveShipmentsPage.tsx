...
export function ActiveShipmentsPage() {
  ...
  const filters = useMemo<ActiveShipmentFilters>(() => ({ jobId: requestedJobId || undefined, lifecycle, risk, assignment, readiness, search, sort, direction: sort === 'created' ? 'desc' : 'asc' }),
    [assignment, lifecycle, readiness, requestedJobId, risk, search, sort]);
  ...
  return (
    <main className="page-stack">
      <PageHeader
        ...
        description="Lifecycle, ETA risk, assigned resources, proof, and invoice readiness within your branch scope."
        actions={<>
          <span className="text-xs text-slate-500" aria-live="polite">
            {data ? `Updated ${formatTime(data.generatedAtUtc)} · ${total} records` : 'Connecting…'}
          </span>
          <button type="button" onClick={() => setReload((value) => value + 1)} disabled={loading} className="btn-ghost">
            <RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} aria-hidden /> Refresh
          </button>
          <button type="button" onClick={handleExport} disabled={!canExport || exporting} title={!canExport ? 'shipments:export is required' : undefined} className="btn-primary">
      ...
      {!canExport && (
        <div className="flex items-start gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-800" role="status">
      ...
      {notice && <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800" role="status" aria-live="polite">{notice}</div>}
      ...
      <section className="panel p-3" aria-label="Active shipment filters">
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-6">
          <label className="text-xs font-bold text-slate-600 xl:col-span-2">Search active shipments
            <span className="relative mt-1 block"><Search className="pointer-events-none absolute left-3 top-2 h-4 w-4 text-slate-400" aria-hidden /><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} maxLength={120} placeholder="Shipment, customer, lane, driver, vehicle, route" className="field w-full pl-9" /></span>
          </label>
        ...
        <div className="mt-3 flex flex-wrap items-end justify-between gap-3 border-t border-slate-100 pt-3">
          <label className="text-xs font-bold text-slate-600">Sort by
            <select value={sort} onChange={(event) => { setSort(event.target.value as ActiveShipmentFilters['sort']); setPage(1); }} className="field ml-2">
          ...
          <button type="button" onClick={() => {
            setLifecycle('All'); setRisk('All'); setAssignment('All'); setReadiness('All');
            setSearchInput(''); setSearch(''); setSort('risk'); setPage(1);
      ...
      <nav aria-label="Related active shipment modules" className="panel p-3">
        <p className="mb-2 text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Continue the workflow</p>
...
function Metric({ label, value, icon, danger, warning }: { label: string; value?: number; icon: ReactElement; danger?: boolean; warning?: boolean }) {
  return <div className={`rounded-xl border px-3 py-2 ${danger ? 'border-red-200 bg-red-50' : warning ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white'}`}><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">{label}</p><span className="[&>svg]:h-3.5 [&>svg]:w-3.5 text-slate-500">{icon}</span></div><p className="mt-0.5 text-xl font-black leading-tight text-slate-950">{value ?? '—'}</p></div>;
}
...
function Filter({ label, value, options, onChange }: { label: string; value: string; options: readonly string[]; onChange: (value: string) => void }) {
  return <label className="text-xs font-bold text-slate-600">{label}<select aria-label={`${label} filter`} value={value} onChange={(event) => onChange(event.target.value)} className="field mt-1 w-full">{options.map((option) => <option key={option} value={option}>{displayToken(option)}</option>)}</select></label>;
}

function ShipmentTableRow({ shipment }: { shipment: ActiveShipment }) {
  return <tr className="align-top hover:bg-slate-50/70">
    <td className="px-4 py-3"><p className="font-black text-slate-900">{shipment.shipmentNumber}</p><p className="mt-0.5 max-w-48 truncate text-xs text-slate-500">{shipment.customerName}</p><StatusBadge status={shipment.priority} /></td>
    <td className="px-4 py-3"><p className="max-w-52 text-xs font-semibold text-slate-700">{shipment.origin} → {shipment.destination}</p><div className="mt-1"><StatusBadge status={shipment.status} /></div></td>
    <td className="px-4 py-3"><span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${riskClass(shipment.riskStatus)}`}>{displayToken(shipment.riskStatus)}</span><p className="mt-1 text-xs font-bold text-slate-700">{formatTime(shipment.etaUtc)}</p><p className="text-[10px] text-slate-500">{etaCopy(shipment)} · {shipment.etaSource}</p></td>
    <td className="px-4 py-3 text-xs"><p className="font-bold text-slate-700">{shipment.driverName || 'Driver needed'}</p><p className="mt-0.5 text-slate-500">{shipment.vehicleNumber || 'Vehicle needed'} · {shipment.routeCode || 'Route pending'}</p><p className="mt-0.5 font-semibold text-slate-600">{displayToken(shipment.assignmentStatus)}</p></td>
    <td className="px-4 py-3 text-xs"><p className="font-bold text-slate-700">{shipment.latestLocation || 'No tracking location'}</p><p className="mt-0.5 text-slate-500">{displayToken(shipment.trackingFreshness)}</p></td>
    <td className="px-4 py-3"><StatusBadge status={shipment.podReady ? 'POD Ready' : 'POD Pending'} /><p className="mt-1 text-[10px] text-slate-500">{shipment.verifiedPods}/{shipment.deliveryStops} delivery stops verified</p></td>
    <td className="px-4 py-3"><StatusBadge status={shipment.isInvoiceReady ? 'Invoice Ready' : 'Invoice Blocked'} /></td>
    <td className="px-4 py-3"><div className="flex flex-col gap-1"><Link to={`/shipments?jobId=${encodeURIComponent(shipment.id)}`} className="text-xs font-black text-teal-700 hover:underline">Lifecycle</Link><Link to={`/proof-of-delivery?jobId=${encodeURIComponent(shipment.id)}`} className="text-xs font-bold text-slate-600 hover:text-teal-700">Proof</Link></div></td>
  </tr>;
}
