...
const KK_CAPABILITIES = [
  ...
  { icon: <Server className="h-5 w-5" />,     label: "Enterprise System Architecture",        color: "text-slate-300",  bg: "bg-slate-400/10 border-slate-400/20" },
    ...
    <div className="space-y-8 pb-8">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div className="panel p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-extrabold text-white">Developed by Kode Kinetics</h2>
          ...
          <div className="space-y-2 pt-2 border-t border-white/[0.07]">
            ...
            <a href="https://www.kodekinetics.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-teal-400 hover:text-teal-300 transition">
              <Globe className="h-3.5 w-3.5" />
              {String(support?.website ?? "www.kodekinetics.com")}
              <ExternalLink className="h-3 w-3 opacity-60" />
            </a>
            <a href="mailto:info@kodekinetics.com" className="flex items-center gap-2 text-sm text-slate-400 hover:text-slate-300 transition">
              <Mail className="h-3.5 w-3.5" />
              {String(support?.email ?? "info@kodekinetics.com")}
            </a>
            <a href="tel:+15714305333" className="flex items-center gap-2 text-sm text-slate-400 hover:text-slate-300 transition">
        ...
        <div className="panel p-6 space-y-4">
          <h2 className="font-extrabold text-white">OpsTrax Platform Modules</h2>
          <div className="grid grid-cols-2 gap-2">
            {PLATFORM_MODULES.map((m) => (
              <div key={m.label} className="flex items-center gap-2 rounded-lg border border-white/[0.05] bg-white/[0.02] px-2.5 py-2">
                <span className={`flex-shrink-0 ${m.color}`}>{m.icon}</span>
                <span className="text-xs font-medium text-slate-300 leading-tight">{m.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="panel p-6 space-y-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h2 className="font-extrabold text-white">Kode Kinetics Capabilities</h2>
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {KK_CAPABILITIES.map((cap) => (
            <div key={cap.label} className={`flex flex-col items-start gap-2 rounded-xl border p-3 ${cap.bg}`}>
              ...
              <p className="text-xs font-semibold text-slate-300 leading-snug">{cap.label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-amber-400/15 bg-amber-400/[0.04] p-4">
        <div className="flex items-start gap-3">
          <ShieldCheck className="h-5 w-5 flex-shrink-0 text-amber-300 mt-0.5" />
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-amber-400 mb-1">Compliance Disclaimer</p>
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm sm:grid-cols-4">
          {[
            ...
            <div key={k}>
              ...
              <p className="font-semibold text-slate-300 mt-0.5">{v}</p>
            </div>
          ))}
        </div>
      </div>
