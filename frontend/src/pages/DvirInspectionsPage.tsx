...
export function DvirInspectionsPage() {
  ...
  const action = useMutation({
    mutationFn: async ({ kind, id, rowVersion, notes }: { kind: "review" | "certify" | "resolve"; id: number; rowVersion: number; notes?: string }) => {
      ...
      return dvirApi.resolveDefect(id, rowVersion, notes?.trim() || "Repair verified in DVIR console");
    },
  ...
  return <div className="mt-5 space-y-5">
    ...
    <section><h3 className="text-sm font-black text-slate-900">Defect evidence ({defects.length})</h3>{defects.length === 0 ? <p className="mt-2 text-sm text-slate-500">No defect rows are attached to this report.</p> : <div className="mt-3 space-y-3">{defects.map((defect) => {
      ...
      return <div key={defectId} className={`rounded-xl border p-4 ${bool(value(defect, "outOfService", "out_of_service")) && open ? "border-red-300 bg-red-50" : "border-slate-200 bg-slate-50"}`}>
        ...
        {open && canClose && <div className="mt-3 flex flex-wrap gap-2"><label className="min-w-60 flex-1"><span className="sr-only">Repair resolution note</span><input className="field h-9" value={resolveNotes[defectId] ?? ""} onChange={(event) => onResolveNotes(defectId, event.target.value)} placeholder="Repair verification note" /></label><button type="button" className="btn-secondary h-9" disabled={busy || !(resolveNotes[defectId] ?? "").trim()} onClick={() => onAction("resolve", Number(defect.id), Number(value(defect, "rowVersion", "row_version")), resolveNotes[defectId])}>Resolve defect</button></div>}
      </div>;
    })}</div>}</section>
