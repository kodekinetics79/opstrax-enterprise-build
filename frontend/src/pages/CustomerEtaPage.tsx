...
export function PublicEtaTrackingPage() {
  ...
  const feedback = useMutation({ mutationFn: () => customerEtaApi.feedback(String((tracking.data?.tracking as AnyRecord)?.id), { trackingCode: params.trackingCode, rating, sentiment: rating >= 4 ? "Positive" : rating >= 3 ? "Neutral" : "Negative", comments: "" }) });
    ...
    <section className="mx-auto max-w-5xl px-6 py-10">
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1fr_340px]">
        <div className="panel p-6">
          <div className="flex items-start justify-between gap-4"><div><p className="text-sm text-slate-500">Tracking code</p><h2 className="text-2xl font-semibold text-slate-950">{String(row.trackingCode)}</h2></div><StatusBadge status={row.status} /></div>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-5">{timeline.map((item)=><div key={String(item.label)} className={`rounded-2xl border p-4 ${item.complete ? "border-teal-200 bg-teal-50 text-teal-800" : "border-slate-200 bg-slate-50 text-slate-600"}`}><p className="text-sm font-semibold">{String(item.label)}</p></div>)}</div>
          <div className="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-4"><h3 className="section-title">Proof of Delivery Preview</h3><p className="mt-3 text-sm text-slate-700">Proof status: {String(row.proofStatus || "Pending")}</p><p className="mt-1 text-sm text-slate-500">Signature/photo links are intentionally hidden until proof is captured.</p></div>
        </div>
        <aside className="panel p-6"><h2 className="section-title">Customer Message</h2><p className="mt-3 text-slate-700">{String(data?.customerMessage)}</p><div className="mt-6"><RiskBadge risk={row.slaStatus} /><p className="mt-3 text-sm text-slate-600">SLA risk explanation: {row.slaStatus === "At Risk" ? "This service is being actively monitored due to ETA variance." : "This service is tracking within the expected SLA window."}</p></div><div className="mt-6"><label htmlFor="delivery-rating" className="text-sm text-slate-700">Rate this delivery (1-5 stars)</label><input id="delivery-rating" className="field mt-2" type="number" min="1" max="5" value={rating} onChange={(e)=>setRating(Number(e.target.value))} /><button className="btn-primary mt-3" onClick={()=>feedback.mutate()}>Submit Feedback</button></div></aside>
      </div>
    </section>
  </main>;
}
