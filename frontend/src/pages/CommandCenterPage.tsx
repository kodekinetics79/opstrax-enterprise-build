...
export function CommandCenterPage() {
  ...
  return (
    <div className="space-y-5">
      {/* ── Command banner ─────────────────────────────────── */}
      <header className="relative overflow-hidden rounded-[20px] border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950 px-6 py-5 shadow-xl">
        <div className="pointer-events-none absolute -right-16 -top-24 h-72 w-72 rounded-full bg-teal-500/10 blur-3xl" />
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2.5">
              <span className="inline-flex items-center gap-1.5 rounded-md bg-white/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.22em] text-teal-200 ring-1 ring-white/10">
              ...
              <span className="text-[11px] font-semibold text-slate-300">Live · refreshes every 15s</span>
              {isFetching && <RefreshCw className="h-3 w-3 animate-spin text-teal-300" />}
            </div>
            <h1 className="mt-2 text-2xl font-black tracking-tight text-white sm:text-[26px]">
              Command Center
            ...
            <p className="mt-1.5 max-w-2xl text-sm text-slate-300">
              {critCount > 0
                ? <><span className="font-bold text-red-300">{critCount} critical</span> and {warnCount} warning signal{warnCount === 1 ? "" : "s"} on the board — act on the queue before the next dispatch window.</>
                : warnCount > 0
                  ? <><span className="font-bold text-amber-300">{warnCount} warning{warnCount === 1 ? "" : "s"}</span> in play — fleet is holding but watch the exception queue.</>
          ...
          <div className="flex flex-col items-end gap-3">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => navigate("/alerts")}
                className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3.5 py-2 text-xs font-bold text-slate-900 shadow-sm transition hover:bg-slate-100">
                <ShieldCheck className="h-3.5 w-3.5" /> Acknowledge Risks
              </button>
              <button type="button" onClick={() => exportCsv("command-center", kpis)}
                className="inline-flex items-center gap-1.5 rounded-lg border border-white/15 bg-white/5 px-3.5 py-2 text-xs font-semibold text-slate-200 transition hover:bg-white/10">
