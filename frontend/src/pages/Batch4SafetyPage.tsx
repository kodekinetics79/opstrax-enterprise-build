...
const configs = {
  ...
  evidence: {
    ...
    description: "Create a branch-scoped draft from recorded incident evidence references. A package cannot be locked until every included item passes retrieval, content-hash and provenance verification.",
    ...
    fields: [["incidentId","Incident ID"],["safetyEventId","Safety Event ID"],["dashcamEventId","Dashcam Event ID"],["driverId","Driver ID"],["vehicleId","Vehicle ID"],["jobId","Job ID"],["summary","Summary"]],
    // Retrieval verification is not yet exposed as a user workflow. The backend
    // therefore keeps lock fail-closed and this UI does not advertise it.
    actions: [],
...
export function Batch4SafetyPage({ kind }: { kind: Kind }) {
  ...
  return <div className="fleet-console space-y-3">
    ...
    <div className="panel flex flex-wrap divide-x divide-slate-100" aria-label={`${config.eyebrow} summary`}>{config.kpis.slice(0, 5).map(([label,key]) => <KpiCard compact key={key} label={label} value={kind === "dashcam" ? (typeof s[key] === "number" && Number.isSafeInteger(s[key]) && Number(s[key]) >= 0 ? String(s[key]) : "Unavailable") : kind === "safety" && key === "fleetSafetyScore" && !hasSourceRecords ? "Not assessed" : String(s[key] ?? 0)} status={/critical|overdue|missing|rejected/i.test(label) ? "Critical" : kind === "safety" && key === "fleetSafetyScore" && !hasSourceRecords ? "Unknown" : undefined} />)}</div>
    <div className="panel grid gap-2 p-3 sm:grid-cols-[minmax(0,1fr)_180px]">
      <input aria-label={`Search ${config.eyebrow.toLowerCase()} records`} className="field" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={`Search ${config.eyebrow.toLowerCase()} by driver, vehicle, route, event, status...`} />
      <select aria-label={`Filter ${config.eyebrow.toLowerCase()} records by status`} className="field" value={filter} onChange={(e) => setFilter(e.target.value)}><option>All</option><option>Critical</option><option>High</option><option>Pending</option><option>Reviewed</option><option>Open</option><option>Closed</option><option>Locked</option></select>
    </div>
    {!rows.length ? (
