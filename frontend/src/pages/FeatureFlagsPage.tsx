...
import { ErrorState, LoadingState, EmptyState, PageHeader, DataTable } from "@/components/ui";
      ...
      {rows.length === 0 ? (
        ...
        <DataTable rows={rows} columns={["name", "flagKey", "enabled", "rolloutPct", "environment", "description"]}
          ...
          actions={canManage ? (f) => <>
            ...
            <button type="button" className="btn-ghost btn-compact text-red-600" aria-label={`Delete ${String(f.name)}`} disabled={mut.isPending} onClick={() => { if (confirm(`Delete flag "${String(f.flagKey)}"?`)) mut.mutate({ action: "delete", key: String(f.flagKey) }); }}><Trash2 className="h-3.5 w-3.5" /></button>
          </> : undefined}
        />
      )}
