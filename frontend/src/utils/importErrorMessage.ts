...
/**
 * Extracts only customer-safe fields from a handled API rejection. Diagnostic
 * response fields and stack traces are deliberately ignored.
 */
export function importErrorMessage(error: unknown, fallback: string): string {
  ...
  // Some import adapters reject with an Error created from a successful HTTP
  // envelope. Use its message, but never Axios's generic status text or a stack.
